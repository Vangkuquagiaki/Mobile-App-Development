...
class _HomeScreenState extends State<HomeScreen>
  ...
  // Server trả về đường dẫn tương đối (ví dụ /uploads/avatars/x.jpg);
  // ghép với baseUrl để Image.network có thể tải được.
  ...
  Future<void> _pickAndUploadAvatar(ImageSource source) async {
    ...
    final success = await authProvider.updateAvatar(File(picked.path));
    if (!mounted) return;
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(success
            ...
            : (authProvider.errorMessage ?? 'Cập nhật ảnh đại diện thất bại')),
        backgroundColor: success ? const Color(0xFF2AA95C) : const Color(0xFFDB3022),
  ...
  Future<void> _removeAvatar() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(success
            ? 'Đã xóa ảnh đại diện'
            : (authProvider.errorMessage ?? 'Không thể xóa ảnh đại diện')),
        backgroundColor: success ? const Color(0xFF2AA95C) : const Color(0xFFDB3022),
        behavior: SnackBarBehavior.floating,
      ),
    );
        ...
        return SafeArea(
          child: Column(
            ...
            children: [
              ...
              ListTile(
                leading: const Icon(Icons.photo_camera_outlined, color: AppTheme.textPrimary),
              ...
              if (hasAvatar)
                ListTile(
                  leading: const Icon(Icons.delete_outline, color: Color(0xFFDB3022)),
                  title: const Text('Xóa ảnh đại diện', style: TextStyle(color: Color(0xFFDB3022))),
                  onTap: () {
                    Navigator.pop(ctx);
                    _removeAvatar();
                  },
  ...
  Widget _buildEditableAvatar(String? avatarUrl, String initials) {
    final hasAvatar = avatarUrl != null && avatarUrl.isNotEmpty;
    return GestureDetector(
      onTap: _isUploadingAvatar ? null : () => _showAvatarOptions(hasAvatar),
      child: Stack(
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Image.asset(
          ...
          height: 72,
          fit: BoxFit.cover,
          // Nếu thiếu khai báo asset trong pubspec.yaml, vẫn fallback
          // về vòng tròn chữ cái đầu để app không bị crash.
  ...
  Widget _buildProfileCard(
    ...
    return Container(
      ...
      child: Row(
        children: [
          // Avatar
          _buildEditableAvatar(avatarUrl, initials),
