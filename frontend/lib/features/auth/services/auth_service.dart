...
import 'package:nguyen_minh_thien/features/auth/models/auth_response.dart';
import 'package:nguyen_minh_thien/features/auth/models/user_response.dart';
...
class AuthService {
  ...
  Future<String> uploadAvatar(File imageFile) async {
    final token = await getAccessToken();
    final uri = Uri.parse('${AppConstants.baseUrl}/api/users/avatar');
    ...
    if (response.statusCode == 200) {
      ...
      final body = jsonDecode(response.body);
      throw Exception(body['message'] ?? 'Cập nhật ảnh đại diện thất bại');
    }
  }
  ...
  Future<void> deleteAvatar() async {
    ...
    final response = await http.delete(
      Uri.parse('${AppConstants.baseUrl}/api/users/avatar'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
    ...
    if (response.statusCode != 200) {
      throw Exception('Không thể xóa ảnh đại diện');
    }
  }
